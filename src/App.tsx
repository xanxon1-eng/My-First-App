...
import { SpecProgressWidget } from './components/SpecProgressWidget';
import { useTrainingCore } from './core/TrainingCore';

export default function App() {
  const [activeBottomPanel, setActiveBottomPanel] = useState<'console' | 'diagnostics' | 'spec-progress'>('spec-progress');
    ...
    <div className="flex flex-col h-full w-full bg-kingfisher-dark text-kingfisher-surface font-sans overflow-hidden">
      ...
      <header className="h-14 border-b border-kingfisher-border bg-kingfisher-panel flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-semibold tracking-wide text-sm hidden sm:block text-white">Kingfisher App</h1>
        </div>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-kingfisher-dark relative">
          ...
          <div className="h-1/3 md:h-64 flex flex-col bg-kingfisher-panel shrink-0">
            ...
            <div className="flex items-center border-b border-kingfisher-border px-2">
              <button 
                className={`px-4 py-2 text-xs font-medium border-b-2 transition-colors ${activeBottomPanel === 'spec-progress' ? 'border-kingfisher-blue text-white' : 'border-transparent text-kingfisher-muted hover:text-white'}`}
                onClick={() => setActiveBottomPanel('spec-progress')}
              >
                <div className="flex items-center gap-1.5"><LayoutDashboard className="w-3.5 h-3.5" /> Spec Progress</div>
              </button>
              <button 
            ...
            <div className="flex-1 overflow-hidden relative">
              {activeBottomPanel === 'spec-progress' && <SpecProgressWidget />}
