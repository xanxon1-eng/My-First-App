                ...
                <div className="max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                  <ul className="space-y-3 pt-1">
                    {[
                      ...
                      ['Shallow Water Equations (SWE) & Fluid Solver Mathematics', 'Saint-Venant formulations integrating height and average horizontal velocity fields to simulate physical wave propagation, obstacle reflection, dynamic circular wakes, and depth-averaged viscosity in high-end open-world biomes.'],
                      ['RPG Hardware Budget Gaps & Platform Latency Specs (PC & Consoles)', 'A detailed profiling database measuring the precise GPU, CPU, System RAM, VRAM, and dynamic network ping impact of advanced hydrology solvers on modern hardware.'],
                      ['Unreal Engine 5.5 Support & Niagara Fluid Integrations', 'Step-by-step documentation mapping native UE5 hydrology mechanisms (such as spline-based systems and Niagara grids) against technical voids (such as lack of dynamic lod limits and multi-joint skeletal drag integration).'],
                      ['High-Performance HLSL Shallow Water Shader & Math Discretization (Custom Material & Niagara GPU)', 'Production-grade HLSL material node shader matching depth values to custom normal displacements, wave-slope divergence foam vectors, and G-Buffer refraction UV offsets on staggered grids to completely eliminate standard Game Thread culling overhead (saves -12.5ms CPU, consuming +1.15ms GPU and allocating 16MB VRAM).'],
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    ...
                    <ul className="space-y-5">
                      {[
                        [
                          'Real-Time 3D Volumetric SPH Foam Spray Cascades (GPU Compute Fluid Auxiliary Solver)',
                          'A highly optimized 3D Smoothed Particle Hydrodynamics solver running exclusively on GPU compute threads to generate secondary volumetric foam splashes and curling crest droplets when dynamic river currents slam into wide obstacles, running fully decoupled from the main SWE height solver.',
                          'CPU: 0ms | GPU: +4.20ms Niagara compute passes (high fluid flow velocities) | RAM: +12.0MB dynamic particle buffers | VRAM: +35.0MB render targets | Latency: 0ms',
                          'Unreal Engine lacks dynamic world-coordinate bounding clip-margins in standard Niagara Fluids templates. Developers must write custom Niagara Grid interface readers in C++ to stream velocity values from the 2D SWE solver into a sparse 3D particle emitter system.'
                        ],
                        [
